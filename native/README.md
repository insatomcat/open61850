...
The `core/` crate of this workspace, `open61850-core`, holds the process bus codecs (Sampled Values decoding, GOOSE encoding and decoding) for real-time callers: `no_std`, no allocation, no unsafe code, the same accepted input and the same output octets as the Python codecs of open61850.

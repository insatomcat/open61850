...
from open61850.sv_publisher import Fault, Publisher, SvStream, Wave, build_template, render_frame, three_phase
...
def _random_stream(rng: random.Random) -> SvStream:
    ...
    return SvStream(sv_id=f"SV{rng.randint(0, 99)}", app_id=rng.randint(0, 0xFFFF), dst_mac="01:0c:cd:04:00:01",
                    src_mac="02:00:00:00:00:01", waves=waves, fault=fault,
...
def test_native_render_matches_python(seed: int) -> None:
    ...
    engine = rt.Engine("lo", rate, asdus, _native_streams(streams, templates), 0)
    for _ in range(200):
        second = rng.choice([0, 1_790_000_000, 2_000_000_000]) + rng.randint(0, 20)
...
def test_native_rejects_bad_input() -> None:
    ...
    spec = _native_streams([stream], [build_template(stream, 2, 4800)])
